...
class ConsumableController extends Controller
{
    ...
    public function update(Consumable $consumable) {
        $data = request()->validate([
            'name' => ['required', 'string'],
            'price' => ['required', 'numeric', 'between:0,99999.99'],
            'crypto_id' => ['required', 'numeric', 'exists:cryptos,id'],
        ...
        // Replace image if it is updated
        if (!is_null($data['image'])) {
            $data['image'] = $data['image']->store('consumable', 'public');
        }


        //Create an consumable in the database
        $consumable->update($data);

        //Create skill effects for consumable
        $consumable->skill_effects()->delete();
        foreach($data['skill'] as $key=>$value) {
            if(!is_null($value)) {
                $consumable->skill_effects()->create(
                    [
                        'skill_id' => $key,
                        'formula' => $value,
                    ]
        ...
        //Create vital effects for consumable
        $consumable->vital_effects()->delete();
        foreach($data['vital'] as $key=>$value) {
            if(!is_null($value)) {
                $consumable->vital_effects()->create(
                    [
                        'vital_id' => $key,
                        'formula' => $value,
                    ]
