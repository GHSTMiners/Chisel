                ...
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="{{ route('trait-effect.vitals.update', [$traitEffect, $traitVitalEffect]) }}" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                <label for="description" class="form-label">{{ __('Vital') }}</label>
                                <select class="form-select" aria-label="vital_id" name="vital_id">
                                @foreach ($vitals as $currentVital)
                                    <option value="{{$currentVital->id}}" @if($currentVital->id == $traitVitalEffect->vital_id) selected @endif>{{$currentVital->name}}</option>
                                @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input id="multiplier" name="multiplier" type="number" placeholder="0.00" step="0.001" class="form-control @error('name') is-invalid @enderror" value="{{ old('multiplier') ?? $traitVitalEffect->multiplier }}" >
                                @error('multiplier')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="mb-3">
                                ...
                                <input id="offset" name="offset" type="number" placeholder="0.00" step="0.001" class="form-control @error('name') is-invalid @enderror" value="{{ old('offset') ?? $traitVitalEffect->offset}}" >
                                @error('offset')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
